...
Collection.prototype.search = function search(criteria, cb, indices) {
	...
	self.client.search({
		index: indices || self.elasticSearch.index,
		type: self.identity,
		body: criteria,
	}, function (err, docs) {
...
Collection.prototype.insert = function index(values, parent, cb) {
	var self = this,
			id = values[self.primaryKey];

	var options = {
		index: self.elasticSearch.index,
		type: self.identity,
		id: id,
		body: values
	}
	...
	self.client.create(options, function (err, docs) {
		if(err) return cb(err);
		cb(null, docs);
	});
};
...
Collection.prototype.update = function index(id, values, parent, cb) {
	var self = this;
	...
	self.client.update(options, function (err, docs) {
		console.log(err);
		if(err) return cb(err);
		cb(null, docs);
	});
